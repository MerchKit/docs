...
<Steps>
  ...
  <Step title="Learn your keys">
    A product's fields are **attributes**, defined per workspace rather than hard-coded. So your
    first real call is schema discovery — it tells you every key you can read and write, its data
  ...
  <Step title="Read a product">
    Every entity — list row, detail read, or mutation response — uses the same **one-map shape**:
    system fields at the top level, every customer-defined key inside `attributes`. References come
    back as labeled stubs, so you never do a second lookup just to render a name:
  ...
  <Step title="Find anything by SKU">
    ...
    That's the whole recipe — a bare `filter[key]=value` means equals. The full
...
<Callout type="info">
  **The UUID is the address. Your SKU is a filter.** Item paths take UUIDs only
  ...
  `?filter[sku]=ARIA-DT-72` — or resolve 100 at once with `?filter[sku][in]=A,B,C`. And when you'd
...
<Steps>
  ...
  <Step title="Sources are async">
    A **source** is a web page or document Merchkit scrapes for enrichment. `POST /v1/sources` with
    a URL returns `202` and a `job_id` — processing is asynchronous. Poll `GET /v1/jobs/{job_id}`
    until `completed`, then find the created source via `GET /v1/sources?filter[url]=...`. Attaching
...
The headline workflow: a 5,000-SKU delta push is **50 calls**, with no ID-mapping table and no
create-vs-update fork on your side.
...
Note the deliberate typo in the third item (`pricee`) — it shows what a failed item looks like:
...
Three habits make batch integrations robust:

- **Check `summary.has_errors` first** — clean batches (the steady-state majority) need no per-item scan, and `summary.failed` is your log line and alerting metric.
...
- **Route retries on `error.retriable`, never on message text.** `retriable: false` means the same item will fail the same way — dead-letter it for a human; `retriable: true` (rate limits, transient server errors) belongs in your retry queue. Don't sum `summary` counts across re-runs of the same batch: an idempotent retry reports `updated` for rows the first attempt `created`.

Each item is matched on `sku` (trimmed, case-insensitive) and created or updated accordingly, with a
**per-item status** — re-running the same batch is idempotent. Check every item: an HTTP `200` does
**not** mean every item succeeded; failed items carry an `error` instead of an `id`. Up to 100 items
...
what happened, and both outcomes return the full resource — no follow-up `GET` needed:
...
First call — no product carries that SKU yet, so one is **created** (`201 Created`):
...
Re-run the same call with a changed price and the existing product is **updated** (`200 OK`) — same
`id`, merged attributes, bumped `updated_at`:
...
<ParamField path="OpenAPI spec" type="string">
  `https://www.merchkit.com/api/v1/openapi` — the machine-readable contract, with worked examples on
...
<CardGroup cols={2}>
  ...
  <Card title="MCP server" icon="robot" href="/developers/mcp">
    Connect Claude, ChatGPT, or VS Code in one click — same shapes as REST.
  </Card>
</CardGroup>
