...
<Note>
  The examples below use `https://app.merchkit.com` as the base URL. Treat this as a placeholder and
  confirm your account's actual API host before going to production.
</Note>
...
<ParamField path="Base URL" type="string">
  `https://app.merchkit.com/api/v1` — all REST endpoints live under the `/api/v1` prefix.
</ParamField>

<ParamField path="OpenAPI spec" type="string">
  `https://app.merchkit.com/api/v1/openapi` — the machine-readable contract. Field and enum
...
<ParamField path="llms.txt" type="string">
  `https://app.merchkit.com/llms.txt` — a curated, instruction-bearing guide written for agents. It
