---
title: "Errors"
description: "Every API error code — meaning, HTTP status, retriability, an example body, and how to fix it — plus the full catalog of field-level issue codes."
---

Every non-2xx response from `/api/v1` uses one machine-recoverable envelope. The `code` is stable
and machine-readable; `documentation_url` links to the matching section on this page; and
`field_errors` (when present) name the exact field or filter clause at fault.
...
<Callout type="info">
  `field_errors[].acceptable_values` is the strongest self-correction signal in the API: when a
  select value is rejected, the response carries the exact set that would have passed, so an agent
  can fix the payload and retry without a human.
</Callout>
...
The request was understood but rejected — a malformed body, an unknown attribute key, a bad filter
clause, or a value that fails its data type or acceptable values. `field_errors` names each fault.
...
Too many requests. Limits are enforced at the edge during the beta — no fixed numbers are
published.
...
The operation contradicts existing data. The two cases: an **upsert** matched more than one
existing entity (the message carries the matching ids), or a write hit a uniqueness constraint.
...
write validator can raise. (One exception: when a request **body fails its structural schema** —
