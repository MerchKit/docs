...
| `not_a_defined_attribute` | A filter, sort, `fields`, write, or `merge_key` names a key that doesn't exist for this type | Discover keys via `GET /v1/attributes?type=…` |
...
| `system_field_always_returned` | [`fields`](/developers/conventions#selecting-fields) names a system field (`id`, `type`, `label`, `parent_id`, `created_at`, `updated_at`) | Drop it — system fields are always returned; `fields` selects attribute keys only |
| `not_selectable_inverse_reference` | `fields` names an `inverse_reference` key | Inverse references never appear in `attributes` — read them via `GET /{resource}/{id}/references?attribute=…` |
