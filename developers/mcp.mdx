...
snake_case arguments, and runs the same validation — so nothing an agent learns from these docs is
MCP-specific, and nothing drifts.
...
The tools are **generic over entity type** — one `list_entities` instead of a tool per resource,
